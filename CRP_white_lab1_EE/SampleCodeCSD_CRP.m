...
Reference = 2;
DataCommands=Reference*ones(N0,1); %Vector storing the input sent to the plant. 
...
while cond==1
    input=Reference; %Input of the system (current).     ++++++++++++++
%     for i 1:(
    if (i*Ts > 0.5)
        DataCommands(i) =  Reference +0.5 ;
    end 
    anaout(DataCommands(i),0); %Command to send the input to the analog computer.
