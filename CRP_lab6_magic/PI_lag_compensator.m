...
%Derive the continous and discrete lag controller of Ring
% form Dp(s) = k* (kp_r*s + ki)/(s+0.5); where ki> 0.5; kp_r has to be 1
kp_m = 4;% motor controller
ki =1.4;%1.834 %0.496;
kp_r =1;%1.31 %0.251;
k = 1; %0.853       % gain factor of ring controller
Dp_s = tf([kp_r ki],[1 0.5]); % Phase lag controller % tranfer function of ring controller in continous 
Dp_d = c2d(Dp_s,Ts,'Tustin'); % discrete transfer function of ring controller i
b = Dp_d.num{1}; %[b0 b1]
a = Dp_d.den{1}; % [a0 a1] 
...
while cond==1
    ...
    % put control law                         
    Data(i,2) = in2; %     the real position
...
%     end
    ...
    ref_speed = (-a(2)*prev_speed_command + b(1)*error + b(2)*prev_error); % reference velocity in rad/s small signal; and b(1) is b0 a(2) is a1
